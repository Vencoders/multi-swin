...
class Block(nn.Module):
    def __init__(
        self, n_feats, kernel_size, block_feats, res_scale=1, act=nn.ReLU(True)):
        super(Block, self).__init__()
        self.res_scale = res_scale
        # device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        # self.alpha.append(torch.nn.Parameter(torch.Tensor([1.0]), requires_grad=True).to(device))
        # self.beta.append(torch.nn.Parameter(torch.Tensor([0.2]), requires_grad=True).to(device))
        body = []
        body.append(
            torch.nn.utils.weight_norm(nn.Conv2d(n_feats, block_feats, kernel_size, padding=kernel_size//2)))
        body.append(act)
        body.append(
            torch.nn.utils.weight_norm(nn.Conv2d(block_feats, n_feats, kernel_size, padding=kernel_size//2)))
...
class MultiSwinTransformerBlock(nn.Module):
    ...
    def forward(self, query, hw_shape):
        ...
        if shift_size > 0 and self.shift_mode == 's':
            query = torch.roll(
                query, shifts=(-shift_size, -shift_size), dims=(1, 2))
            attn_mask = self.get_attn_mask((H_pad, W_pad),
                                           window_size=window_size,
        ...
        if shift_size > 0 and self.shift_mode == 'h':
            query = torch.roll(
                query, shifts=-shift_size, dims=1)
            attn_mask = self.get_h_attn_mask((H_pad, W_pad),
                                             window_size=window_size,
                                             shift_size=shift_size,
                                             device=query.device)

        if shift_size > 0 and self.shift_mode == 'w':
            query = torch.roll(
                query, shifts=-shift_size, dims=2)
            attn_mask = self.get_w_attn_mask((H_pad, W_pad),
                                             window_size=window_size,
                                             shift_size=shift_size,
                                             device=query.device)
        ...
        shifted_x = self.window_reverse(attn_windows, H_pad, W_pad,
                                        window_size)
        # reverse cyclic shift
        if self.shift_size > 0 and self.shift_mode == 's':
            x = torch.roll(
                shifted_x, shifts=(shift_size, shift_size), dims=(1, 2))
        elif self.shift_size > 0 and self.shift_mode == 'h':
            x = torch.roll(
                shifted_x, shifts=shift_size, dims=1)
        elif self.shift_size > 0 and self.shift_mode == 'w':
            x = torch.roll(
                shifted_x, shifts=shift_size, dims=2)
        else:
            x = shifted_x
    ...
    @staticmethod
    def get_h_attn_mask(hw_shape, window_size, shift_size, device=None):
        if shift_size > 0:
            img_mask = torch.zeros(1, *hw_shape, 1, device=device)
            w_slices = (slice(0, -window_size), slice(-window_size,
                                                      -shift_size),
    ...
    @staticmethod
    def get_w_attn_mask(hw_shape, window_size, shift_size, device=None):
        if shift_size > 0:
            img_mask = torch.zeros(1, *hw_shape, 1, device=device)
            w_slices = (slice(0, -window_size), slice(-window_size,
                                                      None))
...
class MultiSwinConvParallel(nn.Module):
    """Shift Window Multihead Self-Attention Module.
    Args:
        embed_dims (int): Number of input channels.
        num_heads (int): Number of attention heads.
    ...
    def __init__(self,
        ...
        self.parallelConv = Block(48, 3, 48)
        self.catConv = nn.Sequential(
            nn.Conv2d(96, 48, 3, stride=1, padding=3 // 2),
    ...
    def forward(self, query, hw_shape):
        ...
        y = self.parallelConv(self.up_x4(shortcut, hw_shape))
        x = self.catConv(torch.cat([x.permute(0, 3, 1, 2), y], dim=1))
...
class SelectMultiSwinTransformerBlock(nn.Module):
    """Shift Window Multihead Self-Attention Module.
    Args:
        embed_dims (int): Number of input channels.
        num_heads (int): Number of attention heads.
    ...
    def forward(self, query, hw_shape):
        ...
        if shift_size > 0 and self.shift_mode == 's':
            query = torch.roll(
                query, shifts=(-shift_size, -shift_size), dims=(1, 2))
            query1 = query[:, 0:H_pad // 2, 0:W_pad // 2, :]

        if shift_size > 0 and self.shift_mode == 'h':
            query = torch.roll(
                query, shifts=-shift_size, dims=1)
            query1 = query[:, 0:H_pad // 2, :, :]

        if shift_size > 0 and self.shift_mode == 'w':
            query = torch.roll(
                query, shifts=-shift_size, dims=2)
            query1 = query[:, :, 0:W_pad // 2, :]

        H_attention, W_attention = query1.shape[1], query1.shape[2]
        attn_mask = self.get_attn_mask((H_attention, W_attention),
                                       window_size=window_size,
                                       shift_size=0,
                                       device=query1.device)
        # nW*B, window_size, window_size, C
        query_windows = self.window_partition(query1, window_size)
        # nW*B, window_size*window_size, C
        query_windows = query_windows.view(-1, window_size ** 2, C)
        ...
        # B H' W' C
        shifted_x = self.window_reverse(attn_windows, H_attention, W_attention,
                                        window_size)
        # reverse cyclic shift
        if self.shift_size > 0 and self.shift_mode == 's':
            query[:, 0:H_pad // 2, 0:W_pad // 2, :] = shifted_x
            x = torch.roll(
                query, shifts=(shift_size, shift_size), dims=(1, 2))
        elif self.shift_size > 0 and self.shift_mode == 'h':
            query[:, 0:H_pad // 2, :, :] = shifted_x
            x = torch.roll(
                query, shifts=shift_size, dims=1)
        elif self.shift_size > 0 and self.shift_mode == 'w':
            query[:, :, 0:W_pad // 2, :] = shifted_x
            x = torch.roll(
                query, shifts=shift_size, dims=2)
        else:
            x = shifted_x
...
class SwinConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.pad_small_map = pad_small_map
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.swin_conv = nn.Sequential(
            nn.Conv2d(dim, dim, 3, padding=1),
            nn.GELU()
        )
        self.patch_embed = PatchEmbed_noConv(in_chans=dim, embed_dim=dim, norm_layer=None)
    ...
    def forward(self, query, hw_shape):
        ...
        # FFN
        x = shortcut + self.drop_path(x)
        x = x + (self.patch_embed(self.swin_conv(self.up_x4(self.norm2(x), hw_shape)))[0])
...
class PatchMerging_noconv(nn.Module):
    r""" Patch Merging Layer.
...
class BasicLayer_conv(nn.Module):
    """ A basic Swin Transformer layer for one stage.
...
class BasicLayer_multishift(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    ...
    def __init__(self, dim, depth, num_heads, window_size,
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            MultiSwinTransformerBlock(dim=dim,
                                      num_heads=num_heads, window_size=window_size,
                                      # shift_size=0 if (i % 5 == 0 or i % 5 == 1) else window_size // 2,
                                      shift_size=0 if (i % 4 == 0) else window_size // 2,
                                      # shift_mode=None if (i % 5 == 0 or i % 5 == 1) else 's' if (i % 5 == 2) else 'h' if (i % 5 == 3) else 'w',
                                      shift_mode=None if (i % 4 == 0) else 's' if (i % 4 == 1) else 'h' if (i % 4 == 2) else 'w',
                                      mlp_ratio=mlp_ratio,
                                      qkv_bias=qkv_bias, qk_scale=qk_scale,
                                      drop=drop, attn_drop=attn_drop,
                                      drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
...
class BasicLayer_multishiftConv(nn.Module):
    """ A basic Swin Transformer layer for one stage.
